      ...
      <main className="flex-1 ml-[280px] min-h-screen p-8">
        <header className="flex justify-between items-center mb-8">
          <div>
            ...
            <p className="text-gray-500 mt-1">{issues.length} issues reported by you</p>
          </div>
          <div className="flex items-center gap-3">
            <button onClick={fetchIssues} className="p-2.5 bg-white/40 backdrop-blur-sm rounded-xl border border-white/30 hover:bg-white/60 transition-all hover:scale-105">
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <StatCard label="Total Issues" value={counts.ALL} icon={<FiAlertTriangle className="w-5 h-5" />} gradient="bg-gradient-to-r from-indigo-400 to-indigo-500 shadow-indigo-500/30" />
            <StatCard label="Open" value={counts.Open} icon={<FiCircle className="w-5 h-5" />} gradient="bg-gradient-to-r from-blue-400 to-blue-500 shadow-blue-500/30" />
            <StatCard label="In Progress" value={counts['In Progress']} icon={<FiRefreshCw className="w-5 h-5" />} gradient="bg-gradient-to-r from-amber-400 to-amber-500 shadow-amber-500/30" />
            <StatCard label="Resolved" value={counts.Resolved} icon={<FiCheckCircle className="w-5 h-5" />} gradient="bg-gradient-to-r from-emerald-400 to-emerald-500 shadow-emerald-500/30" />
          </div>
          ...
          <div className="flex flex-wrap gap-2">
            {['ALL', 'Open', 'In Progress', 'Resolved', 'Closed'].map((s) => {
              ...
              return (
                ...
                >
                  {s !== 'ALL' && st.icon}
                  {s} <span className={`text-xs ${isActive ? 'text-white/80' : 'text-gray-400'}`}>({counts[s] || 0})</span>
                </button>
              );
            })}
          </div>
          ...
          {loading ? (
            ...
            <div className="text-center py-20 bg-white/30 backdrop-blur-sm rounded-2xl border border-white/30">
              <div className="w-20 h-20 mx-auto bg-gradient-to-r from-rose-100 to-rose-200 rounded-full flex items-center justify-center mb-4">
                <FiAlertTriangle className="w-10 h-10 text-rose-400" />
              </div>
              <h3 className="text-xl font-semibold text-gray-700">No issues found</h3>
              <p className="text-gray-400 mt-1">You haven't reported any issues yet</p>
            </div>
          ) : (
                ...
                <table className="w-full">
                  <thead className="bg-gradient-to-r from-indigo-500/10 to-purple-500/10 backdrop-blur-sm">
                    <tr>
                      ...
                      <th className="px-6 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Reported At</th>
                      ...
                      return (
                        <tr key={item.issueId || item.issue?.issueId || index} className={`hover:bg-white/30 transition-all duration-200 ${index % 2 === 0 ? 'bg-white/20' : 'bg-white/10'}`}>
                          <td className="px-6 py-4">
                            <div className="text-sm font-semibold text-gray-800">{issueTitle || 'No Title'}</div>
                            <div className="text-xs text-gray-500 truncate max-w-[200px]">{item.issueDescription || item.issue?.issueDescription || ''}</div>
                          </td>
                          <td className="px-6 py-4">
                            <div className="text-sm font-medium text-gray-700">{taskName || 'N/A'}</div>
                          </td>
                          <td className="px-6 py-4">
                            <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold ${pr.bg} ${pr.text} border ${pr.border}`}>
                              {pr.icon}
                              {priority}
                            </span>
                          </td>
                          ...
                          <td className="px-6 py-4">
                            <div className="text-sm text-gray-600">{reportedAt ? new Date(reportedAt).toLocaleDateString() : 'N/A'}</div>
                          </td>
        ...
        {showViewModal && viewIssue && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/30 backdrop-blur-sm animate-fadeIn">
            <div className="bg-white/95 backdrop-blur-xl rounded-3xl w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl border border-white/30 animate-slideDown">
              <div className="sticky top-0 bg-white/95 backdrop-blur-xl rounded-t-3xl px-6 py-4 border-b border-gray-100/50 flex justify-between items-center">
                <h3 className="text-xl font-bold bg-gradient-to-r from-rose-600 to-rose-500 bg-clip-text text-transparent flex items-center gap-2">
                  <FiAlertTriangle className="w-5 h-5 text-rose-500" />
                  Issue Details
                </h3>
                <button className="p-1.5 hover:bg-gray-100 rounded-full transition-colors" onClick={() => { setShowViewModal(false); setViewIssue(null); }}>
              ...
              <div className="px-6 py-6">
                <div className="flex justify-between items-start mb-4">
                  ...
                  <div className="flex gap-2">
                    <span className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold ${priorityMeta[viewIssue.issueId !== undefined ? viewIssue.issuePriority : viewIssue.issue?.priority]?.bg || 'bg-gray-100'} ${priorityMeta[viewIssue.issueId !== undefined ? viewIssue.issuePriority : viewIssue.issue?.priority]?.text || 'text-gray-600'} border ${priorityMeta[viewIssue.issueId !== undefined ? viewIssue.issuePriority : viewIssue.issue?.priority]?.border || 'border-gray-200'}`}>
                      ...
                      {viewIssue.issueId !== undefined ? viewIssue.issuePriority : viewIssue.issue?.priority}
                    </span>
                    <span className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold ${issueStatusMeta[viewIssue.issueId !== undefined ? viewIssue.issueStatus : viewIssue.issue?.status]?.bg || 'bg-gray-100'} ${issueStatusMeta[viewIssue.issueId !== undefined ? viewIssue.issueStatus : viewIssue.issue?.status]?.text || 'text-gray-600'} border ${issueStatusMeta[viewIssue.issueId !== undefined ? viewIssue.issueStatus : viewIssue.issue?.status]?.border || 'border-gray-200'}`}>
                      ...
                      {viewIssue.issueId !== undefined ? viewIssue.issueStatus : viewIssue.issue?.status}
                    </span>
                  </div>
                </div>
                ...
                <div className="mb-4">
                  ...
                  <p className="text-gray-600 bg-white/40 backdrop-blur-sm rounded-xl p-4 border border-white/30">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="bg-white/40 backdrop-blur-sm rounded-xl p-4 border border-white/30">
                    ...
                    <p className="text-sm font-semibold text-gray-800">{viewIssue.taskName || viewIssue.title || 'N/A'}</p>
                  </div>
                </div>

                <div className="flex justify-end gap-3 mt-6 pt-4 border-t border-gray-100/50">
                  <button onClick={() => { setShowViewModal(false); setViewIssue(null); }} className="px-4 py-2 bg-gray-100/80 backdrop-blur-sm rounded-full text-gray-700 font-medium hover:bg-gray-200 transition-all text-sm">
                    Close
                  </button>
                </div>
              </div>
