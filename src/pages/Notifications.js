...
function Notifications() {
  ...
  const handleDeleteNotification = async (notificationId) => {
    ...
    setLoading(true);
    try {
      await axios.delete(`https://api.timelyhealth.in/api/tasks/notifications/${notificationId}`);
      fetchNotifications();
  ...
  const handleDeleteSelected = async () => {
    ...
    try {
      for (const notifId of selectedNotifications) {
        await axios.delete(`https://api.timelyhealth.in/api/tasks/notifications/${notifId}`);
      }
      fetchNotifications();
      setSelectedNotifications([]);
      setSelectAll(false);
  ...
  const handleDeleteAllNotifications = async () => {
    ...
    try {
      for (const notif of notifications) {
        await axios.delete(`https://api.timelyhealth.in/api/tasks/notifications/${notif._id}`);
      }
      fetchNotifications();
      setSelectedNotifications([]);
      setSelectAll(false);
