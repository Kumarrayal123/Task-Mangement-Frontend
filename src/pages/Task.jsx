    ...
    const html = `
      ...
      <head>
        ...
        <style>
          ...
          .team-badge { background: #e0e7ff; color: #4338ca; padding: 4px 12px; border-radius: 12px; font-size: 12px; font-weight: 600; }
          ...
          <div class="section">
            ...
            <div class="grid">
              <div class="field">
                ...
                <div class="field-value"><span class="team-badge">${reportData.team.teamName || 'N/A'}</span></div>
              </div>
              <div class="field">
                <div class="field-label">Team Lead</div>
                <div class="field-value">${reportData.team.teamLead?.name || 'N/A'}</div>
              </div>
              <div class="field">
                <div class="field-label">Members</div>
                <div class="field-value">${reportData.team.members?.map(m => m.name).join(', ') || 'N/A'}</div>
              </div>
              <div class="field">
                <div class="field-label">Status</div>
                <div class="field-value">${reportData.team.status || 'N/A'}</div>
              </div>
            </div>
          </div>
          ` : ''}
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-3 sm:p-4 bg-black/30 backdrop-blur-sm animate-fadeIn">
      <div className="bg-white/95 backdrop-blur-xl rounded-2xl sm:rounded-3xl w-full max-w-4xl max-h-[90vh] overflow-y-auto shadow-2xl border border-white/30 animate-slideDown">
        ...
        <div className="px-4 sm:px-8 py-4 sm:py-6">
          ...
          {/* Team Info Section */}
          {selectedTask.team && (
            <div className="bg-gradient-to-r from-emerald-50/80 to-teal-50/80 backdrop-blur-sm rounded-xl p-3 sm:p-4 border border-emerald-200/50 mb-4 sm:mb-6">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <div className="flex items-center gap-2 sm:gap-3">
                  <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-gradient-to-r from-emerald-500 to-teal-500 flex items-center justify-center text-white font-bold text-xs sm:text-sm shadow-lg shadow-emerald-500/30">
                    {typeof selectedTask.team === 'object' ? selectedTask.team.teamName?.charAt(0).toUpperCase() : 'T'}
                  </div>
                  <div>
                    <p className="text-xs sm:text-sm font-semibold text-gray-800 flex items-center gap-1.5">
                      <FiUsers className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-emerald-600" />
                      {typeof selectedTask.team === 'object' ? selectedTask.team.teamName : 'Team'}
                    </p>
                    <div className="flex flex-wrap items-center gap-1 sm:gap-2 text-[10px] sm:text-xs">
                      <span className="text-gray-500">
                        {typeof selectedTask.team === 'object' && selectedTask.team.description ? selectedTask.team.description : 'No description'}
                      </span>
                      <span className="px-1.5 sm:px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-700 text-[8px] sm:text-[10px] font-medium">
                        <FiUsers className="inline mr-0.5 w-2.5 h-2.5" />
                        {typeof selectedTask.team === 'object' && selectedTask.team.members ? selectedTask.team.members.length : 0} members
                      </span>
                      {typeof selectedTask.team === 'object' && selectedTask.team.teamLead && (
                        <span className="text-[8px] sm:text-[10px] text-gray-400 flex items-center gap-0.5">
                          <FiUser className="w-2.5 h-2.5" />
                          Lead: {selectedTask.team.teamLead.name || 'Unknown'}
                        </span>
                      )}
                    </div>
                  </div>
                ...
                <div className="flex items-center gap-1.5">
                  {typeof selectedTask.team === 'object' && selectedTask.team.status && (
                    <span className={`px-2 sm:px-3 py-0.5 sm:py-1 rounded-full text-[10px] sm:text-xs font-semibold ${
                      selectedTask.team.status === 'active' ? 'bg-emerald-100 text-emerald-700' : 'bg-gray-100 text-gray-700'
                    }`}>
                      {selectedTask.team.status === 'active' ? '✅ Active' : '⛔ Inactive'}
                    </span>
                  )}
                  {typeof selectedTask.team === 'object' && selectedTask.team.department && (
                    <span className="px-2 sm:px-3 py-0.5 sm:py-1 rounded-full text-[10px] sm:text-xs font-semibold bg-indigo-100 text-indigo-700">
                      <FiBriefcase className="inline mr-0.5 w-2.5 h-2.5" />
                      {selectedTask.team.department}
                    </span>
                  )}
                </div>
              </div>
              
              {typeof selectedTask.team === 'object' && selectedTask.team.members && selectedTask.team.members.length > 0 && (
                <div className="mt-2 pt-2 border-t border-emerald-200/50">
                  <p className="text-[10px] sm:text-xs font-semibold text-gray-600 mb-1.5 flex items-center gap-1">
                    <FiUsers className="w-3 h-3" />
                    Team Members
                  </p>
                  <div className="flex flex-wrap gap-1.5">
                    {selectedTask.team.members.map((member, idx) => (
                      <span key={idx} className="inline-flex items-center gap-1 px-2 py-0.5 bg-white/60 backdrop-blur-sm rounded-full text-[8px] sm:text-[10px] text-gray-700 border border-gray-200/50">
                        <div className="w-4 h-4 rounded-full bg-gradient-to-r from-indigo-400 to-purple-400 flex items-center justify-center text-white font-bold text-[6px] sm:text-[8px]">
...
function Task() {
  ...
  const fetchTasks = async () => {
    ...
    try {
      ...
      const employeeCreated = tasksData.filter(t => t.createdByType === 'employee').length;
      const adminCreated = tasksData.filter(t => t.createdByType === 'admin' || !t.createdByType).length;
    ...
    if (filterTeam !== 'all') {
      filtered = filtered.filter((task) => {
        if (task.team && task.team._id) {
          return task.team._id === filterTeam;
        }
        if (typeof task.team === 'string') {
          return task.team === filterTeam;
        }
        return false;
      });
    }
      ...
      <main className="flex-1 w-full px-4 sm:px-6 lg:px-8 py-6">
        <div className="admin-dash">
          ...
          <div className="admin-dash__header">
            <div>
              <h1 className="admin-dash__greeting flex items-center gap-2">
                <FaTasks className="w-5 h-5 text-indigo-600" /> Task <span>Management</span>
              </h1>
              <p className="admin-dash__subtitle">
                Create, assign, track, and manage workforce tasks and employee assignments.
              </p>
            </div>
          ...
          <div className="space-y-6">
            ...
            {stats && (
              <div className="admin-dash__stats grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-4">
                <div className="admin-dash__stat cursor-pointer" onClick={() => { setFilterStatus('all'); setFilterDue('all'); }}>
                  ...
                  <div className="admin-dash__stat-value">{stats.total || 0}</div>
                ...
                <div className="admin-dash__stat cursor-pointer" onClick={() => { setFilterStatus('Pending'); setFilterDue('all'); }}>
                  ...
                  <div className="admin-dash__stat-value">{stats.pending || 0}</div>
                ...
                <div className="admin-dash__stat cursor-pointer" onClick={() => { setFilterStatus('In Progress'); setFilterDue('all'); }}>
                  ...
                  <div className="admin-dash__stat-value">{stats.inProgress || 0}</div>
                ...
                <div className="admin-dash__stat cursor-pointer" onClick={() => { setFilterStatus('Completed'); setFilterDue('all'); }}>
                  ...
                  <div className="admin-dash__stat-value">{stats.completed || 0}</div>
                ...
                <div className="admin-dash__stat cursor-pointer" onClick={() => { setFilterStatus('Overdue'); setFilterDue('all'); }}>
                  ...
                  <div className="admin-dash__stat-value">{stats.overdue || 0}</div>
                  <div className="admin-dash__stat-meta">past deadline</div>
                </div>

                <div className="admin-dash__stat cursor-pointer" onClick={() => { setFilterDue('upcoming'); setFilterStatus('all'); }}>
                  <div className="admin-dash__stat-top">
                  ...
                  <div className="admin-dash__stat-value">{getUpcomingTasksFilter(tasks).length}</div>
                  <div className="admin-dash__stat-meta">due in 7 days</div>
                </div>
              </div>
            )}
            ...
            <div className="admin-dash__card">
              <div className="admin-dash__card-header">
                <div>
                  <h3 className="admin-dash__card-title">Task Directory</h3>
                  <p className="admin-dash__card-desc">Filter, search, and manage workforce task assignments</p>
                </div>
                <div className="flex items-center gap-2">
              ...
              <div className="admin-dash__card-body space-y-4">
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-3">
                    ...
                    <div className="flex items-center gap-2.5 flex-wrap">
                      ...
                      >
                        ...
                        <option value="employee">Employee Created</option>
                      </select>
                      ...
                      >
                        ...
                        <span className={`text-[10px] ${filterDue === 'upcoming' ? 'text-white/80' : 'text-slate-400'}`}>
                          ({getUpcomingTasksFilter(tasks).length})
                        </span>
                      </button>

                      {(searchTerm || filterStatus !== 'all' || filterPriority !== 'all' || filterDue !== 'all' || filterCreatedBy !== 'all' || filterFrequency !== 'all' || filterTeam !== 'all') && (
                        <button
                          onClick={() => {
                            setSearchTerm('');
                            setFilterStatus('all');
                ...
                {loading ? (
                  <div className="flex flex-col items-center justify-center py-20 bg-white rounded-2xl border border-slate-200 shadow-xs">
                    ...
                    <p className="mt-3 text-xs font-semibold text-slate-500">Loading tasks...</p>
                  </div>
                  ...
                  <div className="text-center py-16 bg-white rounded-2xl border border-slate-200 shadow-xs p-6">
                    ...
                    <p className="text-xs text-slate-400 mt-1 max-w-sm mx-auto">
                      {filterDue === 'upcoming' 
                        ...
                        : filterTeam !== 'all'
                        ? `No tasks found for team: ${teams.find(t => t._id === filterTeam)?.teamName || 'Unknown'}`
                        : filterFrequency !== 'all'
                      ...
                      <table className="w-full text-left border-collapse">
                        <thead>
                          <tr className="bg-slate-50/80 border-b border-slate-200 text-[11px] font-bold text-slate-500 uppercase tracking-wider">
                            ...
                            <th className="py-3.5 px-4 sm:px-6">Team</th>
                        ...
                        <tbody className="divide-y divide-slate-100 text-xs">
                          {currentTasks.map((task, index) => {
                            ...
                            let teamInitial = 'T';
                            if (task.team) {
                              if (typeof task.team === 'object' && task.team.teamName) {
                                teamName = task.team.teamName;
                                teamInitial = teamName.charAt(0).toUpperCase();
                              } else if (typeof task.team === 'string') {
                            ...
                            return (
                              <tr
                                ...
                                className={`hover:bg-slate-50/80 transition-colors cursor-pointer ${
                                  isUpcoming && daysLeft <= 3 ? 'border-l-4 border-l-amber-400 bg-amber-50/20' : ''
                                }`}
                                onClick={() => openViewModal(task)}
                              >
                                <td className="py-3.5 px-4 sm:px-6">
                                  <div className="font-semibold text-slate-800 truncate max-w-[180px]">{task.taskName}</div>
                                  <div className="text-[11px] text-slate-400 truncate max-w-[180px]">{task.title}</div>
                                  {isUpcoming && daysLeft <= 3 && (
                                    <span className="inline-flex items-center gap-1 text-[10px] text-amber-600 font-bold mt-1">
                                      ...
                                      {daysLeft <= 0 ? 'Overdue!' : `${daysLeft} day(s) left`}
                                    </span>
                                  )}
                                </td>
                                ...
                                <td className="py-3.5 px-4 sm:px-6 whitespace-nowrap">
                                  {task.team ? (
                                    <div className="flex items-center gap-1.5">
                                      <div className="w-6 h-6 rounded-full bg-gradient-to-r from-indigo-500 to-purple-500 flex items-center justify-center text-white font-bold text-[8px] flex-shrink-0">
                                        {teamInitial}
                                      </div>
                                  ...
                                  <div className="flex flex-wrap gap-1">
                                    {task.frequency && task.frequency.length > 0 ? (
                                      task.frequency.map((freq, idx) => (
                                        <span key={idx} className="inline-flex items-center gap-1 px-2.5 py-0.5 bg-indigo-50 text-indigo-700 border border-indigo-200/60 rounded-full text-[10px] font-semibold">
                                ...
                                <td className="py-3.5 px-4 sm:px-6 whitespace-nowrap">
                                  <div className="flex items-center gap-2 min-w-[100px]">
                                    ...
                                    <span className="text-[11px] font-bold text-slate-700">{task.progress || 0}%</span>
                                  </div>
                                </td>
                                ...
                                <td className="py-3.5 px-4 sm:px-6 whitespace-nowrap">
                                  <div className="flex items-center gap-2">
                                    <div className={`w-7 h-7 rounded-full flex items-center justify-center text-white font-bold text-[10px] border-2 border-white shadow-2xs ${
                                      isEmployeeCreated ? 'bg-emerald-600' : 'bg-indigo-600'
                                    }`}>
                                      {creatorInitial}
                                    </div>
                                    ...
                                    <div className="flex items-center gap-1">
                                      <div className="flex -space-x-1.5">
                                        {task.assignedTo.slice(0, 3).map((user, idx) => (
                                          <div key={idx} className="w-7 h-7 rounded-full bg-indigo-600 text-white font-bold text-[10px] flex items-center justify-center border-2 border-white shadow-2xs" title={typeof user === 'object' ? user.name : 'Employee'}>
                                            {typeof user === 'object' ? user.name?.charAt(0).toUpperCase() : 'U'}
                                          </div>
                                        ))}
                                        {task.assignedTo.length > 3 && (
                                          <div className="w-7 h-7 rounded-full bg-slate-200 text-slate-700 font-bold text-[10px] flex items-center justify-center border-2 border-white shadow-2xs">
                                            +{task.assignedTo.length - 3}
                                          </div>
                                        )}
                                      </div>
